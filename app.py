...
with st.sidebar:
    ...
    line-height:1.4;'> 🏫  Lớp: Khoa học Dữ liệu trong Kinh doanh<br> 🎓  Trường: Đại học Ngân hàng TP.Hồ Chí Minh (HUB)</p>
    </div>
    """, unsafe_allow_html=True)

# Lọc cơ sở dữ liệu thuần túy theo Chức danh công việc đã chọn
...
if current_friction > 0.3:
    badge_html = '<div class="badge-container"><span class="badge-status" style="background:#fee2e2; color:#ef4444;"> 🚨  Ma Sát Cao (Thiếu hụt Công nghệ)</span></div>'
elif current_friction < -0.3:
...
with tab1:
    ...
    # --- KỊCH BẢN INSIGHT ĐỘNG CHO TAB 1 ---
    if selected_job == "Toàn bộ ngành KHMT":
        insight_1_html = "• <b>Cấu trúc vĩ mô toàn ngành:</b> Thị trường phân hóa rõ rệt giữa nhóm nghiên cứu chuyên sâu (lương cao, quy mô hẹp) và nhóm kỹ sư vận hành (lương trung bình, quy mô lớn lên tới hơn 4 triệu lao động).<br>• <b>Điểm nghẽn hệ thống:</b> Dữ liệu tổng hợp chỉ ra các vị trí có thu nhập càng cao thì kích thước bong bóng động lực càng lớn, phản ánh áp lực thặng dư thời gian của nhóm chất xám cao cấp."
    elif current_friction > 0.3:
        insight_1_html = f"• <b>Cảnh báo thâm hụt công nghệ:</b> Phân khúc vị trí <span class='highlight-navy'>{selected_job}</span> đang có chỉ số ma sát rất cao (<span class='highlight-pink'>{current_friction:.2f} điểm</span>). Nhóm nhân sự này sở hữu mức thu nhập trung bình năm đạt <span class='highlight-pink'>${avg_w:,.2f} USD</span> với tổng quy mô đại diện là <span class='highlight-pink'>{emp_count:,.0f} lao động</span>.<br>• <b>Bản chất kinh doanh:</b> Người lao động ở vị trí này đang bị quá tải nghiêm trọng bởi các tác vụ lặp đi lặp lại có tính chất căng thẳng, trong khi các giải pháp công nghệ hiện hành chưa hỗ trợ tương xứng. Đây là cơ hội vàng để doanh nghiệp đầu tư ngân sách triển khai AI Agent tự trị nhằm chiếm lĩnh lợi thế vận hành tinh gọn."
    elif current_friction < -0.3:
        insight_1_html = f"• <b>Vùng thặng dư công nghệ:</b> Khác biệt với các vị trí khác, vị trí <span class='highlight-navy'>{selected_job}</span> có chỉ số ma sát âm (<span class='highlight-pink'>{current_friction:.2f} điểm</span>). Nhóm nhân sự này sở hữu mức thu nhập trung bình năm đạt <span class='highlight-pink'>${avg_w:,.2f} USD</span> với tổng quy mô đại diện là <span class='highlight-pink'>{emp_count:,.0f} lao động</span>.<br>• <b>Bản chất kinh doanh:</b> Áp lực công việc của nhóm này tương đối cân bằng. Năng lực giải quyết thực tế của các mô hình AI hiện hành đang đáp ứng vượt mong đợi của người lao động. Chiến lược tối ưu ở đây không phải là nhồi nhét thêm AI, mà là duy trì tính ổn định của hệ thống hiện tại để bảo toàn chi phí."
    else:
        insight_1_html = f"• <b>Trạng thái cân bằng động:</b> Vị trí <span class='highlight-navy'>{selected_job}</span> đang nằm trong vùng tối ưu hóa lý tưởng với điểm ma sát ổn định (<span class='highlight-pink'>{current_friction:.2f} điểm</span>). Nhóm nhân sự này sở hữu mức thu nhập trung bình năm đạt <span class='highlight-pink'>${avg_w:,.2f} USD</span> với tổng quy mô đại diện là <span class='highlight-pink'>{emp_count:,.0f} lao động</span>.<br>• <b>Bản chất kinh doanh:</b> Tốc độ phát triển năng lực của AI Agent đồng nhịp hoàn toàn với nhu cầu giảm tải công việc của kỹ sư. Mô hình vận hành tại đây đạt độ chín muồi tốt nhất trong danh mục đầu tư."
    st.markdown(f'<div class="insight-box"><b> 💡  Khảo sát chuyên sâu phân khúc <span class="highlight-navy">{selected_job}</span>:</b><br>{insight_1_html}</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    # --- KỊCH BẢN INSIGHT ĐỘNG CHO TAB 2 ---
    if selected_job == "Toàn bộ ngành KHMT":
        insight_2_html = f"• <b>Tâm lý học lao động vĩ mô:</b> Tỷ lệ lớn nhân sự toàn khối KHMT sẵn sàng chuyển giao các tác vụ tẻ nhạt cho AI. Tuy nhiên, mức độ lo ngại rủi ro mất việc (AI Suffering) phân hóa mạnh ở các nhóm kỹ sư lập trình thô.<br>• <b>ROI vĩ mô:</b> Nếu áp dụng đồng bộ {slider_auto}% tự động hóa trên quy mô lớn, tổng dòng tiền tối ưu hóa sẽ đạt con số khổng lồ là ${money_saved/1000000:,.1f} Triệu USD."
    elif money_saved > 20000000:
        insight_2_html = f"• <b>Hành vi nhân sự nhóm quy mô lớn:</b> Nhóm <span class='highlight-navy'>{selected_job}</span> có số lượng nhân sự đông, tâm lý e ngại AI thường tỷ lệ thuận với khối lượng công việc lặp lại hằng ngày.<br>• <b>Hiệu ứng đòn bẩy kinh tế:</b> Việc chuyển giao <span class='highlight-pink'>{slider_auto}%</span> tác vụ giúp giải phóng tới <span class='highlight-pink'>{hours_saved:,.0f} giờ/năm</span>. Nhờ quy mô nhân sự lớn, giá trị dòng tiền giữ lại đạt mức tác động vĩ mô <span class='highlight-pink'>${money_saved/1000000:,.2f} Triệu USD</span>. Doanh nghiệp nên dùng ngân sách này tái cấu trúc hệ thống hơn là sa thải nhân sự."
    else:
        insight_2_html = f"• <b>Hành vi nhân sự nhóm chuyên sâu:</b> Nhân sự thuộc nhóm <span class='highlight-navy'>{selected_job}</span> ít lo sợ bị thay thế hơn vì tính chất công việc đòi hỏi tư duy logic cao, họ coi AI là công cụ giải phóng sức lao động.<br>• <b>Giá trị thặng dư chiều sâu:</b> Khoản thặng dư tối ưu hóa dòng tiền đạt <span class='highlight-pink'>${money_saved:,.2f} USD</span>. Do quy mô nhân sự đặc thù tinh gọn, mục tiêu tối ưu ở đây không phải cắt giảm chi phí lao động đại trà, mà là nâng cao năng suất, giúp giải phóng áp lực để kỹ sư tập trung vào thiết kế kiến trúc lõi hệ thống."
    st.markdown(f'<div class="insight-box"><b> 💡  Biện luận định lượng ROI cho mục nghiên cứu <span class="highlight-navy">{selected_job}</span>:</b><br>{insight_2_html}</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with col_r2:
        ...
        # --- KỊCH BẢN INSIGHT ĐỘNG CHO TAB 3 ---
        if selected_job == "Toàn bộ ngành KHMT":
            insight_3_html = f"• <b>Đánh giá Khoảng cách Chiến lược vĩ mô:</b> Nhìn chung toàn ngành, khả năng đáp ứng thực tế của công nghệ AI đạt điểm chuyên gia trung bình ở mức khá (<span class='highlight-pink'>{ai_readiness/20:.2f}/5</span>).<br>• <b>Khuyến nghị phân lớp:</b> Doanh nghiệp cần phân rã phòng ban theo ma trận 3 tầng tác vụ để đạt hiệu suất tối ưu, không áp dụng một mô hình duy nhất cho tất cả các vị trí chức danh."
        elif current_friction > 0.3:
            insight_3_html = f"• <b>Khoảng cách kỳ vọng (Expectation Gap):</b> Biểu đồ mạng nhện bộc lộ rõ khoảng lệch lớn ở góc 'Tránh lỗi con người' and 'Giảm tải áp lực'. Khả năng đáp ứng thực tế của AI hiện tại chỉ đạt <span class='highlight-pink'>{ai_readiness/20:.2f}/5 điểm</span> chuyên gia, thấp hơn nhiều so với kỳ vọng.<br>• <b>Chistven lược triển khai:</b> Đối với <span class='highlight-navy'>{selected_job}</span>, giải pháp triển khai bắt buộc phải cấu hình dưới dạng <b>Autonomous Agent (Hệ thống Agent tự trị quy trình)</b> để chủ động kiểm soát lỗi ngầm độc lập, giảm thiểu tối đa sự can thiệp thủ công."
        else:
            insight_3_html = f"• <b>Độ tương thích năng lực:</b> Biểu đồ mạng nhện của nhóm <span class='highlight-navy'>{selected_job}</span> cho thấy độ tương thích cực kỳ cao, năng lực của AI đạt mức <span class='highlight-pink'>{ai_readiness/20:.2f}/5 điểm</span> chuyên gia, bao trọn hầu hết các đỉnh kỳ vọng của người lao động.<br>• <b>Chiến lược triển khai:</b> Doanh nghiệp không cần tốn chi phí xây dựng hệ thống tự trị độc lập phức toàn quyền. Mô hình khuyến nghị tối ưu nhất tại đây là <b>Symbiotic Copilot (Trợ lý ảo cộng sinh đồng hành)</b> giúp kỹ sư tối ưu hóa thời gian xử lý công việc hằng ngày một cách mượt mà."
        st.markdown(f'<div class="insight-box"><b> 📋  Đánh giá Khoảng cách Chiến lược cho nhóm <span class="highlight-navy">{selected_job}</span>:</b><br>{insight_3_html}</div>', unsafe_allow_html=True)
